...
const BlogList = ({ className = "", id, poster, title, content, date }) => {
  ...
  return (
    <div
      onClick={handleClick}
      className={`flex flex-col cursor-pointer items-start justify-start gap-[20px] max-w-full text-left text-xl text-darkorange-200 font-dm-sans transform transition-transform duration-300 hover:scale-105 ${className}`}
    >
      ...
      <div
        style={{ maxWidth: "min-content", minWidth: "340px" }}
        className="text-justify flex flex-col items-start justify-start mq750:!pr-5"
      >
...
BlogList.propTypes = {
  ...
  poster: PropTypes.string.isRequired, // Array of image URLs
