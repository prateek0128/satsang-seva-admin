...
const SideNavbar = forwardRef(({ isOpen, toggleNav, closeNav, isMobile }, ref) => {
  ...
  useEffect(() => {
    ...
    const fetchCounts = async () => {
      try {
        const res = await axios.get(`${url}admin/badge-counts`, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const navItems = [
    { to: "/admin/dashboard",       label: "Overview",        icon: <DashboardIcon    fontSize="small" /> },
    { to: "/admin/allusers",        label: "Users",           icon: <UsersIcon        fontSize="small" /> },
    { to: "/admin/events",          label: "Events",          icon: <EventsIcon       fontSize="small" /> },
    { to: "/admin/approvals",       label: "Approvals",       icon: <ApproveIcon      fontSize="small" /> },
    { to: "/admin/drafts",          label: "Drafts",          icon: <DraftsIcon       fontSize="small" /> },
    { to: "/admin/blog",            label: "Blogs",           icon: <BlogIcon         fontSize="small" /> },
    { to: "/admin/bookings",        label: "Bookings",        icon: <BookingsIcon     fontSize="small" />, badge: 0 },
  ...
  return (
    ...
    >
      ...
      {/* Nav Items */}
      <nav className="snav" style={{ flex: 1, padding: "10px 8px", overflowY: "auto", display: "flex", flexDirection: "column", gap: 2 }}>
        {isOpen && (
          <div style={{ padding: "6px 8px 4px", marginBottom: 2 }}>
            <span style={{ fontSize: "0.56rem", fontWeight: 800, color: "rgba(255,255,255,0.2)", textTransform: "uppercase", letterSpacing: "0.14em" }}>
              Main Menu
            </span>
          </div>
        )}
        ...
        {navItems.map(({ to, label, icon, badge }) => {
          ...
          const linkEl = (
            ...
            >
              ...
              {isOpen && (
                <>
                  <span className="snav-label" style={{ fontSize: "0.875rem", fontWeight: active ? 700 : 500, color: active ? "#fff" : "rgba(255,255,255,0.6)", flex: 1, fontFamily: "'Plus Jakarta Sans',sans-serif", letterSpacing: "-0.01em" }}>
                    {label}
                  </span>
                  <Badge count={badge} />
                </>
          ...
          return collapsed
            ? <Tooltip key={to} title={label} placement="right" arrow>{linkEl}</Tooltip>
      ...
      <div style={{ padding: "10px 8px 14px", borderTop: "1px solid rgba(255,255,255,0.06)", flexShrink: 0 }}>
        {collapsed ? (
          ...
          <button className="logout-btn" onClick={handleLogOut} style={{ justifyContent: "flex-start" }}>
            <LogoutIcon style={{ fontSize: "1.1rem", flexShrink: 0 }} />
