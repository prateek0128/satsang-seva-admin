...
const Chip = ({ icon, label, value }) => (
  <div className="flex flex-col gap-0.5 bg-gray-50 hover:bg-orange-50 border border-gray-100 hover:border-orange-200 rounded-xl p-2 transition-all">
    ...
    <p className="text-gray-900 font-bold text-sm pl-5 leading-snug truncate">
      {value || "—"}
    </p>
  </div>
);
...
const Event = () => {
    ...
    return (
      ...
      >
        ...
        >
          ...
          <p style={{ color: "#888", fontSize: "0.95rem", fontWeight: 500 }}>
            Loading event...
          </p>
        </div>
      </div>
  ...
  if (!event)
    return (
      ...
      >
        <p style={{ color: "#555", fontSize: "1.1rem", fontWeight: 600 }}>
          Event not found
        </p>
        <Link
          to="/"
      ...
      >
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          >
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
              <Chip
                icon="📅"
                label="Starts"
                value={`${dayjs(event.startDate).format("DD MMM YYYY")}${event.startTime ? " • " + event.startTime : ""}`}
              />
              <Chip
                icon="🏁"
                label="Ends"
                value={`${dayjs(event.endDate).format("DD MMM YYYY")}${event.endTime ? " • " + event.endTime : ""}`}
              />
              ...
              {event.sponserName && (
                <Chip icon="🤝" label="Sponsor" value={event.sponserName} />
              )}
              {event.city && (
                <Chip
                  icon="📍"
                  label="City"
                  value={`${event.city}${event.province ? ", " + event.province : ""}`}
                />
              )}
              <Chip
                icon="🎟️"
                label="Entry"
                value={isFree ? "Free" : `₹${event.eventPrice}`}
              />
            </div>
            {event.hostWhatsapp && (
              <a
                href={`https://wa.me/91${event.hostWhatsapp}`}
                target="_blank"
          ...
          >
            ...
            <p
              style={{ color: "#222", fontSize: "0.875rem", marginBottom: 12 }}
            >
          ...
          >
            ...
            >
              {[
                {
                  icon: "📅",
                  val: dayjs(event.startDate).format("DD MMM YYYY"),
                },
                event.startTime && { icon: "🕐", val: event.startTime },
                event.city && {
                  icon: "📍",
                  val: `${event.city}${event.province ? `, ${event.province}` : ""}`,
                },
